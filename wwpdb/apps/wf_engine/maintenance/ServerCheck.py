        ...
        for row in list:
            ...
            if row[2] > 0:
                ...
                if (row[3] - row[4] - row[5]) / row[2] > 0.9:
                    ...
                    self.sendHelpEmails(
                        "High memory usage on " + str(row[0]),
                        "High physical memory usage on "
                        + str(row[0])
                        + " for SITE-ID = "
                        + str(siteID)
                        + " has a fraction of %8.2f memory usage" % (row[3] / (row[2] - row[4] - row[5])),
                    )
                else:
            ...
            if row[7] / row[6] > 0.8:
                ...
                self.sendHelpEmails(
                    "CPU usage on " + str(row[0]), "CPU usage on " + str(row[0]) + " for SITE-ID = " + str(siteID) + " has usage %8.2f out of %d" % (row[4], row[5])
                )
            else:
                if row[7] / row[6] > 0.5:
