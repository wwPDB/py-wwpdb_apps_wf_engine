...
class AutoRun(object):
    __queries = {
        "LAST_STATUS_AND_ORDINAL": ("select status, ordinal from communication "
                                    "where parent_dep_set_id = '{dep_id}' "
                                    "order by actual_timestamp desc limit 1"),

        "START_ANNOTATION_WORKFLOW": ("UPDATE communication set sender='WFM', receiver='WFE', dep_set_id='{dep_id}', "
